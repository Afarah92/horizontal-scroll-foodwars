...
class Smooth {
  ...
  setBounds(elems) {
    ...
    elems.forEach((el, index) => {
      ...
      if (this.data.total === index && elems === this.dom.elems[0]) {
        ...
        document.querySelectorAll("#scroll-wrapper")[0].style.height = `${w}px`;
      }
    });
  }

  resize() {
    this.setBounds(this.dom.elems[0]); // this.setBounds(this.dom.elems[1]);
  ...
  run() {
    ...
    const skew = velo * 7.5;
    this.dom.content[0].style.transform = `translate3d(-${this.data.last.one.toFixed(2)}px, 0, 0) skewX(${skew}deg)`; // this.dom.content[1].style.transform = `translate3d(-${this.data.last.two.toFixed(
    //   2
